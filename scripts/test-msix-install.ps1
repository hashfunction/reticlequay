...
$package=Get-ChildItem (Join-Path $sourceRoot 'build/AimWisp_*_x64.msix') | Select-Object -First 1
if (-not $package) { throw 'Unsigned MSIX is missing.' }
if (Get-AppxPackage -Name $identity) { throw 'An AimWisp installation already exists; refusing to replace it.' }
...
$testPackage=Join-Path $sourceRoot 'build/AimWisp-install-test.msix'
$certificatePath=Join-Path $sourceRoot 'build/AimWisp-install-test.cer'
$certificate=$null
$installed=$null
$passed=$false
try {
    Copy-Item -LiteralPath $package.FullName -Destination $testPackage
    $certificate=New-SelfSignedCertificate -Type Custom -KeyUsage DigitalSignature -CertStoreLocation 'Cert:\CurrentUser\My' -TextExtension @('2.5.29.37={text}1.3.6.1.5.5.7.3.3','2.5.29.19={text}') -Subject $publisher -FriendlyName 'AimWisp ephemeral CI installation test' -NotAfter (Get-Date).AddDays(1)
    ...
    $env:RETICLEQUAY_EXECUTABLE=Join-Path $installed.InstallLocation 'AimWisp.exe'
    ...
    if (-not $passed -and $installed) {
        Get-Process AimWisp -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
